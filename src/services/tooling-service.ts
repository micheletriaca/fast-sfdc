...
import { extractInvalidApexClassNames, extractMissingFields, extractMissingRelationships } from '../utils/apex-errors'
...
export default {
  ...
  repairApexDependencies: async (error: any, importedClassNames: string[] = []): Promise<boolean> => {
    ...
    if (!invalidClassNames.length && !missingFields.length && !missingRelationships.length) return false
    ...
    if (!unmanagedClassNames.length) return false

    const apexClasses = await sfdcConnector.findApexClassesByNames(unmanagedClassNames)
    ...
    if (!classesToCompile.length) return false

    for (const missingRelationship of missingRelationships) {
      const describe = await sfdcConnector.describeSObject(missingRelationship.entityName)
    ...
    for (const missingField of missingFields) {
      const describe = await sfdcConnector.describeSObject(missingField.entityName)
