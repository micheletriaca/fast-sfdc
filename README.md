...
[![Buy Me a Coffee](https://img.shields.io/badge/Buy_Me_a_Coffee-FFDD00.png?logo=buymeacoffee&logoColor=000)](https://buymeacoffee.com/micheletriaca)
[![GitHub Sponsor](https://img.shields.io/github/sponsors/micheletriaca.png?label=Sponsor&logo=GitHub)](https://github.com/sponsors/micheletriaca)
...
- **Keep every org one click away.** Add, switch, replace or remove Production, Sandbox and custom-domain credentials, and configure deploy on save or read-only mode independently for each org. Each credential has a unique alias, while multiple credentials can share an environment when they need the same plugin and patch behaviour. Read-only orgs allow retrieve and local editing but block remote changes; writable production orgs let you validate before deploying, require confirmation and let you choose the Apex test level when tests are required. Credentials live in an encrypted project vault; its encryption key stays in the operating-system keychain, so the project never contains readable secrets.
